...
class MMWHSDataset(Dataset):
    ...
    def _check_image_files(self):
        # TODO: tilpas til MMWHS eller slet!
        ...
        if self._check_if_valid_file(image_file):
            try:
                ...
                if org_size == img_size:
                    ...
                    label_X = (label_in == 420).astype(np.float32)
                else:
                    ...
                    if self.n_classes == 2: #(Background + Foreground)
                        label_X = (label_in>0).astype(np.float32)
                    else:
                        label_X = np.zeros(label_in.shape).astype(np.float32)
                        label_X[label_in == 500] = 1 #LV
                        label_X[label_in == 600] = 2 #RV
                        label_X[label_in == 420] = 3 #LA
                        label_X[label_in == 550] = 4 #RA
...
#                sitk.WriteImage(img_sitk,debug_dir+image_file[-17:-13]+"_image.nii.gz")
#                lab_sitk = sitk.GetImageFromArray(label_X)
#                sitk.WriteImage(lab_sitk,debug_dir+image_file[-17:-13]+"_label.nii.gz")
#                
#                print("Writing debugging image and label")
                ...
                print("File ", image_file, " raises exception")
                print("ValueError")
        return img_in, label_X, org_size
...
#        print("filename: ", file_name)
        ...
        image_file = os.path.join(self.image_dir,file_name+'_image.nii.gz')
        label_file = os.path.join(self.label_dir,file_name+'_label.nii.gz')
        ...
        if self.n_classes == 2: #(Background + foreground)
            ...
            #label = np.expand_dims(label_in,axis=3)
            #label = label_in
