...
class RHDataset_dfield(Dataset):
    """
    ...
    def _safe_read_and_scale_image(self, image_file, label_file, img_size):
        ...
        if self._check_if_valid_file(image_file):
            try:
                ...
                #label_in = label_in[:,:,::-1] #TODO: OBS  OBS  OBS     Dette er et gipsy fix!! Fix dfields istedet OBS OBS OBS OBS
                ...
                if not org_size == img_size:
                    ...
                    label_in = self.resample_sitk(label_itk, img_size, interpolator = sitk.sitkLinear)

                img_in = np.clip(img_in,-1000,1000)/1000      
                label_in = np.clip(label_in,-5,5)/5
...
#        print("filename: ", file_name)
        ...
        label_file = os.path.join(self.label_dir,file_name+'.nii')
        ...
        label = label_in.reshape(self.image_size, self.image_size, self.image_size, self.n_channels)
