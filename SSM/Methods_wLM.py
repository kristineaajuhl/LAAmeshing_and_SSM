...
def DistanceFieldRegistration(args):
    ...
    elastix_dir = args[10]
    dirLandmarks = args[11]
    dirSourceLandmarks = args[12]
    ...
    sourceLandmark = os.path.join(dirSourceLandmarks,targetNameList + '.txt')
    ...
    targetLandmark = os.path.join(dirLandmarks,targetNameList + '.txt')
    #------------------------------------------------------------------------------------------------------------------------------------------------------------    
    ...
    subprocess.call([elastix_exe, '-f', sourceDistanceField,'-fMask',sourceMask,'-m',targetDistanceField,'-fp',sourceLandmark, '-mp', targetLandmark,'-p',parameterFile,'-out',outputFolder],stdout=output_pipe)
    #------------------------------------------------------------------------------------------------------------------------------------------------------------        
...
def compute_mean_surface_and_lm(targetNameList,dirSurface,dirLM,dirOutput,sourceName,StatsFile = None,thRMS = 100):
    ...
    for targetName in targetNameList:
        ...
        if np.float(RMS[nameIdx])>thRMS:
            print('To high RMS for surface: %s RMS = %0.2f' % (targetName,np.float(RMS[nameIdx])))
            count += 1
            skibedSurface +=1
            continue    
...
def WriteLandmarkFile(vtkPoints,filename):
    # Write transformed landmarks to .txt file readable by elastix
    numberOfLandmarks = vtkPoints.GetNumberOfPoints()
    npTranformedLandmarks = np.zeros([numberOfLandmarks,3])    
    for index in range(numberOfLandmarks):
        point = [0,0,0]
        vtkPoints.GetPoint(index, point)
